...
@property (nonatomic, copy) RightActionBlock actionBlock;
...
+ (instancetype)showFromImageView:(UIImageView *)imageView withImages:(NSArray *)images atIndex:(NSInteger)index rightActionTitle:(NSString *)actionTitle action:(RightActionBlock)actionBlock {
    
    return [self showFromImageView:imageView withImages:images placeholderImage:[UIImage imageNamed:@"image_default_small"] atIndex:index rightActionTitle:actionTitle action:actionBlock];
}

+ (instancetype)showFromImageView:(UIImageView *)imageView withImages:(NSArray *)images placeholderImage:(UIImage *)image atIndex:(NSInteger)index rightActionTitle:(NSString *)actionTitle action:(RightActionBlock)actionBlock {
    HUPhotoBrowser *browser = [[HUPhotoBrowser alloc] initWithFrame:kScreenRect];
    browser.imageView = imageView;
    browser.images = images;
    browser.imagesCount = images.count;
    ...
    [browser animateImageViewAtIndex:index];
    browser.placeholderImage = image;
    browser.dismissDlock = nil;
    browser.rightActionTitle = actionTitle;
    browser.actionBlock = actionBlock;
...
+ (instancetype)showFromImageView:(UIImageView *)imageView withURLStrings:(NSArray *)URLStrings placeholderImage:(UIImage *)image atIndex:(NSInteger)index dismiss:(DismissBlock)block rightActionTitle:(NSString *)actionTitle action:(RightActionBlock)actionBlock {
    HUPhotoBrowser *browser = [[HUPhotoBrowser alloc] initWithFrame:kScreenRect];
    browser.imageView = imageView;
    browser.URLStrings = URLStrings;
    browser.imagesCount = URLStrings.count;
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    ...
    cell.adjustHeight = self.adjustHeight;
    cell.backgroundImageName = self.backgroundImageName;
    ...
    if (self.URLStrings) {
        UIImage *localImage = [UIImage imageWithContentsOfFile:self.URLStrings[indexPath.row]];
        
        if (localImage) {
            if (localImage.size.width > 2000) {
                cell.imageView.image = [localImage imageCompressForWidth:localImage targetWidth:kWindowWidth*2.0];
            }
            else {
        ...
        if ([self.images[indexPath.row] isKindOfClass:[UIImage class]]) {
            ...
            UIImage *localImage = [UIImage imageWithContentsOfFile:self.images[indexPath.row]];
            ...
            if (localImage) {
                ...
                NSString *itemUrlString = self.images[indexPath.row];
                NSURL *url = [NSURL URLWithString:itemUrlString];
                if (indexPath.row != _index) {
                    [cell.imageView hu_setImageWithURL:url placeholderImage:_placeholderImage completed:^(UIImage *image, NSError *error, NSURL *imageUrl) {
                        if (!error) {
                            [cell.imageView setContentMode:UIViewContentModeScaleAspectFit];
...
- (void)setupTopbar {
    if (_rightActionTitle && _rightActionTitle.length > 0) {
        _topBar = [[UIView alloc] initWithFrame:CGRectMake(0, 10, self.frame.size.width, 44)];
        _topBar.backgroundColor = [UIColor clearColor];
        [self addSubview:_topBar];
        
        CGSize titleSize = [self.rightActionTitle sizeWithAttributes:@{NSFontAttributeName:[UIFont systemFontOfSize:13]}];
        
        UIButton *saveBtn = [UIButton buttonWithType:UIButtonTypeCustom];
        saveBtn.frame = CGRectMake(_toolBar.frame.size.width-(titleSize.width+30.0), 8, titleSize.width+20.0, 28);
        saveBtn.layer.cornerRadius = 2;
        [saveBtn setBackgroundColor:[MainBlueColor colorWithAlphaComponent:0.8]];
        [saveBtn setTitle:self.rightActionTitle forState:UIControlStateNormal];
        [saveBtn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
        saveBtn.titleLabel.font = [UIFont systemFontOfSize:13];
        [saveBtn addTarget:self action:@selector(onAction) forControlEvents:UIControlEventTouchUpInside];
...
- (void)setupToolBar {
    ...
    UIButton *saveBtn = [UIButton buttonWithType:UIButtonTypeCustom];
    saveBtn.frame = CGRectMake(_toolBar.frame.size.width-78, 1, 70, 28);
    saveBtn.layer.cornerRadius = 2;
...
- (void)animateImageViewAtIndex:(NSInteger)index {
    ...
    if (self.URLStrings && !self.images) {
        UIImage *localImage = [UIImage imageWithContentsOfFile:self.URLStrings[_index]];
        
        if (localImage) {
            _imageDidLoaded = localImage != nil;
        }
        else {
